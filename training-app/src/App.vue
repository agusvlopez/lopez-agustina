...
<template>
        ...
        <nav class="bg-black shadow-lg">
            <div class="container mx-auto px-4">
                <div class="flex justify-between items-center py-4">
                    <router-link to="/"><img src="./imgs/logo-blanco.png" alt="Logo Training App"
                            class="maxWidth"></router-link>
                    <div class="hidden md:flex space-x-4">
                        <ul class="flex gap-4 text-white">
                            ...
                            <template v-if="user.rol === 'admin' && user.fullProfileLoaded">
                                <li>
                                    <router-link to="/panel-admin">Panel Admin</router-link>
                                </li>
                            </template>
            ...
            <div v-if="mobileMenuOpen" class="md:hidden">
                ...
                <template v-if="user.id == null">
                    ...
                    <router-link to="/iniciar-sesion" class="block text-white p-2 hover:bg-gray-600">Iniciar
                        sesión</router-link>
                </template>
                <template v-else>
                    <template v-if="user.rol === 'cliente'">
                        <router-link to="/usuario/d6dfuuXe7laEyCh33M0uxKtb9xk1/chat"
                            class="block text-white p-3 hover:bg-gray-600">Chateá con nosotros</router-link>
                    </template>
                    <router-link to="/perfil" class="block text-white p-3 hover:bg-gray-600">Mi perfil</router-link>
                    <form class="block text-white p-3 hover:bg-gray-600" action="" @submit.prevent="handleLogout">
                        <button type="submit">{{ user.email }} (Cerrar sesión)</button>
                    </form>
                </template>
                <template v-if="user.rol === 'admin' && user.fullProfileLoaded">
    ...
    <main>
        ...
        <router-link to="/usuario/d6dfuuXe7laEyCh33M0uxKtb9xk1/chat">
            <div v-if="user.fullProfileLoaded && user.rol === 'cliente'"
